...
// Inicializar la textura estatica
      ...
      alive(true) // La burbuja nace viva
{
    ...
    // Ajustar el origen al centro para que la posicion sea el centro de la burbuja
...
void Bubble::update(float deltaTime) {
    if (!alive) return; // Si ya no esta viva, no hacer nada
    ...
    // Calcular la opacidad basada en el tiempo de vida restante
    // Queremos que se desvanezca al final de su vida
    if (lifeTime <= 0) {
        alive = false; // La burbuja ha muerto
        
    } else {
        // Normalizar lifeTime a un rango para calcular la opacidad
        float normalizedLife = lifeTime / initialLifeTime;
        // Asegurar que normalizedLife esta entre 0 y 1
